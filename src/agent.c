...
{
    struct agent_context *agent = (struct agent_context *)malloc(sizeof(struct agent_context));
    ...
    // reactor for CQ
    agent->reactor = (struct reactor_context *)malloc(sizeof(struct reactor_context));

    // connection resources
    agent->conn_bitmap = (int *)calloc(MAX_CONNECTIONS, sizeof(int));
    agent->conn_id_map = (struct rdma_cm_id **)calloc(MAX_CONNECTIONS, sizeof(struct rdma_cm_id *));

    // key resources
    agent->key_lock_map = (pthread_rwlock_t *)calloc(MAX_TABLE_SIZE, sizeof(pthread_rwlock_t));
    agent->key_fd_map = (int *)calloc(MAX_TABLE_SIZE, sizeof(int));

    for (int i = 0; i < MAX_TABLE_SIZE; i++)
    {
        pthread_rwlock_init(&agent->key_lock_map[i], NULL);
    }
...
struct agent_context *create_server(int node_id, int agent_id, struct conn_param *options)
{
...
void destroy_agent(struct agent_context *agent)
{
    ...
    free(agent->key_lock_map);
    free(agent->key_fd_map);
    free(agent->options);
    free(agent);
...
int add_connection_rc(struct agent_context *agent, char *dst_addr, char *port, struct conn_param *options)
{
    DEBUG_LOG("attempting to add reliable connection to %s:%s.", dst_addr, port);

    int sockfd = find_first_empty_bit_and_set(agent->conn_bitmap, MAX_CONNECTIONS);

    if (sockfd < 0)
    {
        ERROR_LOG("can't open new connection; number of open sockets == MAX_CONNECTIONS.");
        exit(EXIT_FAILURE);
    }
    ...
    ret = rdma_create_id(ec, &id, NULL, RDMA_PS_TCP); // rdma_port_space: RDMA_PS_TCP or RDMA_PS_UDP
    if (ret)
    {
    ...
    DEBUG_LOG("created reliable connection to %s:%s on sockfd #%d.", dst_addr, port, sockfd);

    return sockfd;
}

int add_connection_ud(struct agent_context *agent, char *bind_addr, char *mcast_addr, int is_sender, struct conn_param *options)
{
    ...
    ret = rdma_resolve_addr(id, (bind_rai) ? bind_rai->ai_src_addr : NULL, mcast_rai->ai_dst_addr, TIMEOUT_IN_MS);
    if (ret)
    {
        ERROR_LOG("failed to resolve address, errno: %s.", strerror(errno));
        exit(EXIT_FAILURE);
    ...
    DEBUG_LOG("created unreliable connection to %s on sockfd #%d.", mcast_addr, sockfd);
 ...
 * server side
 */
int accept_connection(struct agent_context *server, struct rdma_cm_id *id, struct conn_param *options)
{
    DEBUG_LOG("attempting to accept reliable connection.");

    int sockfd = find_first_empty_bit_and_set(server->conn_bitmap, MAX_CONNECTIONS);
...
int server_listen(struct agent_context *server, char *src_addr, char *port)
{
    ...
    int listen_fd = find_first_empty_bit_and_set(server->conn_bitmap, MAX_CONNECTIONS);
...
void leave_multicast_group(struct conn_context *ctx)
{
    ...
    ret = rdma_leave_multicast(ctx->id, ctx->mcast_addr);
    if (ret)
    {
...
{
    register_local_mr(ctx, ctx->num_local_mrs, ctx->mr_ctxs);
    register_msg_mr(ctx, MR_NUM);
#ifdef EXP_VERBS
    register_wq(ctx);
#endif
}
...
void on_connect(struct conn_context *ctx)
{
    ...
    {
        add_event_fd(ctx->agent->reactor,
                     EPOLLIN,
                     ctx->comp_channel->fd,
                     ctx,
    ...
    DEBUG_LOG("connection established [sockfd:%d qpnum:%d].", ctx->sockfd, ctx->id->qp->qp_num);
